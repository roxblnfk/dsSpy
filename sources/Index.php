<?php
$wb_crls=array(	'IndexWindow', 'IndexPopUpTray', 'IndexTrayIcon', 'FloatWindow', 'WB_FloatWindow', 'WB_FloatWindow_Timer', 'WB_FloatWindow_Title', 'WB_FloatWindow_Font', 'WB_FloatWindow_Labels', );
$s=1000;foreach($wb_crls as $c){ ++$s;if(!defined($c)) define($c, $s); }unset($s,$c,$wb_crls);
...
class dsSpy{
	...
	static $Options=array('downloadImages'=>true,'autocloseWindow'=>600,'messageAge'=>300,'skin'=>0,'alphaBlend'=>255,'dblClckTime'=>0.2,'freezeMessage'=>300);
	...
	function skin($value, $key=0){ return (is_array(self::$Skins[self::$Options['skin']][$value]) ? 
		(isset(self::$Skins[self::$Options['skin']][$value][$key]) ? self::$Skins[self::$Options['skin']][$value][$key] : self::$Skins[self::$Options['skin']][$value][0]) 
	...
	function init(){
		...
		err_no();
		$x=file_get_contents(DOC_ROOT.'/files/lstpst.dat'); $x=$x ? self::LoadValue($x) : false; self::$LastPost=$x ? $x : array();
		$x=file_get_contents(DOC_ROOT.'/files/wnpstns.dat'); $x=$x ? self::LoadValue($x) : false; self::$WinPositions=$x ? $x : array();
		$x=file_get_contents(DOC_ROOT.'/files/vtrs.dat'); $x=$x ? self::LoadValue($x) : false; self::$Avatars=$x ? $x : array();
		$x=file_get_contents(DOC_ROOT.'/files/ptns.dat'); $x=$x ? self::LoadValue($x) : false; self::$Options=$x ? $x : array('downloadImages'=>true,'autocloseWindow'=>600,'messageAge'=>300,
				'skin'=>0,'alphaBlend'=>255);
		for($ks=array_keys(self::$Avatars),$j=sizeof($ks),$i=0;$i<$j;++$i){
			if(!is_string(self::$Avatars[$ks[$i]])) unset(self::$Avatars[$ks[$i]]);
			if(!is_file(self::$Avatars[$ks[$i]])) unset(self::$Avatars[$ks[$i]]);
		}
		if(!is_dir(DOC_ROOT.'/files/temp')) mkdir(DOC_ROOT.'/files/temp');
		//pre(self::$Avatars);
	...
	function searchNews(&$content){	// поиск чего-то нового
		...
		$old=self::$LastPost;
		self::$LastPost=array();
		$cont=array_values($content);	//theme, themeuri, themeid, author, authorid, authoruri, messageuri, messageid, datetime, cat, caturi
		$r=array();
	...
	function analize(&$cont){
		...
		self::gotWindow($wid, self::$WaitingAvatars['autocloseWindow'],$Title='Новые сообщения ['.$cnt.' шт.]',$Type=1,$a,$x,$y,$w,$h);
	}
	function parse(&$txt){
		$r=array();
			...
			for($a=0,$b=sizeof($cols);$a<$b;++$a){
				...
				switch($a){
					case 1 :
						...
						$r[$i]['themefull']=htmlspecialchars_decode(self::_get_string_chunk($cols[$a],'title="','"',null,true));
					break;
					case 2 : 
						$str2=explode('</a>',$str);
						$r[$i]['author']=trim(html_entity_decode(strip_tags($str2[0])));
						$r[$i]['authoruri']=htmlspecialchars_decode(self::_get_string_chunk($str2[0],'href="','"',null,true));
						$r[$i]['authorid']=intval(self::_get_string_chunk($r[$i]['authoruri'],'u=',null,null,true));
						$r[$i]['messageuri']=htmlspecialchars_decode(self::_get_string_chunk($str2[1],'href="','"',null,true));
						
						$r[$i]['messageid']=intval(self::_get_string_chunk(strtolower($r[$i]['messageuri']),'#post',null,null,true));
					break;
					case 3 : 
					...
					case 4 : 
						...
						$r[$i]['caturi']=htmlspecialchars_decode(self::_get_string_chunk($str,'href="','"',null,true));
					break;
				}
			}
		}
	...
	function wb_gotWindow($Timer=0,$Title='Привет! :)',$Text=array(),$Wdth=220,$Rate=0.75 /*разделитель на 2 столбика*/,$X=null,$Y=null){//© roxblnfk ;)
		...
		if(is_array($Text)){ if(($j=count($Text))>5) $H=50+$j*20; } else $H=200;
		...
		if(isset(self::$wb_windows[$wid])){
			...
			$NEW=true;
			self::$wb_windows[$wid]=wb_create_window(null, NakedWindow, "ololo", $X, $Y, $W, $H, /* WBC_TASKBAR |  */WBC_NOTIFY, /* WBC_DBLCLICK | WBC_HEADERSEL |  */WBC_MOUSEUP);
			//wb_set_visible(self::$wb_windows[$wid], false);
	...
	function gotWindow($wid=null,$Timer=0,$Title='Привет! :)',$Type=0,$TextBlocks=array(),$X=null,$Y=null,$Wdth=360,$Hgnt=500){//© roxblnfk ;)
		...
		// автоскрытие окна
		$function=function($LastEventTime, $wid){  if($LastEventTime==dsSpy::$LastEventTime) WinAPI_USER::AnimateWindow(dsSpy::$wb_windows[$wid]->handle,null,0x00010004);  };
		...
		$b=5;	// border
		$i=0; $x=0;$y=0;$w=$W; $h=20;
		switch($Type){
			case 0 : break; // пустое окно
			case 1 :		// новое сообщение		 theme, themeuri, themeid, author, authorid, authoruri, messageuri, messageid, datetime, cat, caturi
				...
				$onMouseDown=function($id, $button, $shift, $x, $y)use(&$wid,&$OnResize){ WinAPI_USER::GetCursorPos($xx,$yy); $OnResize=array(true,$xx,$yy); };
				$onMouseUp=function($id, $button, $shift, $x, $y)use(&$wid,&$OnResize){
						$OnResize[0]=false; dsSpy::$WinPositions[$wid]=array(dsSpy::$wb_windows[$wid]->x,dsSpy::$wb_windows[$wid]->y,dsSpy::$wb_windows[$wid]->w,dsSpy::$wb_windows[$wid]->h);  };
				...
				self::$wb_controls[$wid][$i]->onMouseMove=function($id, $shift, $x, $y)use(&$wid,&$OnResize){	// перемещение
						if(true!==$OnResize[0]) return false; WinAPI_USER::GetCursorPos($xx1,$yy1); $xx0=$OnResize[1]; $yy0=$OnResize[2]; $OnResize[1]=$xx1; $OnResize[2]=$yy1;
						$nx=dsSpy::$wb_windows[$wid]->x+$xx1-$xx0; $ny=dsSpy::$wb_windows[$wid]->y+$yy1-$yy0;
						$Wa = explode(' ', wb_get_system_info('workarea'));
						if($nx<$Wa[0] || $ny<$Wa[1] || $nx>=$Wa[2] || $ny>=$Wa[3]) return false;
						dsSpy::$wb_windows[$wid]->x=dsSpy::$wb_windows[$wid]->x+$xx1-$xx0; dsSpy::$wb_windows[$wid]->y=dsSpy::$wb_windows[$wid]->y+$yy1-$yy0;
					};
				...
				self::$wb_controls[$wid][$i]->onClick=function($id)use(&$wid){ WinAPI_USER::AnimateWindow(dsSpy::$wb_windows[$wid]->handle,null,0x00010004); };
				...
				self::$wb_controls[$wid][$i]->onMouseDown=function($id, $button, $shift, $x, $y)use(&$wid,&$OnAlpha){ WinAPI_USER::GetCursorPos($xx,$yy); $OnAlpha=array(true,$xx,$yy); };
				...
				self::$wb_controls[$wid][$i]->onMouseMove=function($id, $shift, $x, $y)use(&$wid,&$OnAlpha){	// перемещение
						if(true!==$OnAlpha[0]) return false; WinAPI_USER::GetCursorPos($xx1,$yy1); $xx0=$OnAlpha[1]; $yy0=$OnAlpha[2]; $OnAlpha[1]=$xx1; $OnAlpha[2]=$yy1;
				...
				self::$wb_controls[$wid][$i]->onMouseMove=function($id, $shift, $x, $y)use(&$wid,&$OnResize,&$DownItems){	// расширение
						...
						$Wa = explode(' ', wb_get_system_info('workarea'));
						$xx0=$OnResize[1]; $yy0=$OnResize[2];
						$OnResize[1]=$xx1; $OnResize[2]=$yy1;
						...
						for($i=0,$j=sizeof($DownItems);$i<$j;++$i) dsSpy::$wb_controls[$wid][$DownItems[$i]]->y=dsSpy::$wb_controls[$wid][$DownItems[$i]]->y+$nh-dsSpy::$wb_windows[$wid]->h;
				...
				for($i0=0,$j0=sizeof($TextBlocks);$i0<$j0;++$i0){
					...
					if(is_string(self::$Avatars[$Text['authorid']]))
						if(is_file(self::$Avatars[$Text['authorid']])) dsSpy::$wb_controls[$wid][$i]->picture->loadFromFile(self::$Avatars[$Text['authorid']],'png');
					...
					self::$wb_controls[$wid][$i]->onMouseEnter=function($id){ c($id)->font->color=dsSpy::skin('DateTimeColor',1); };
					self::$wb_controls[$wid][$i]->onMouseLeave=function($id){ c($id)->font->color=dsSpy::skin('DateTimeColor',0); };
					// name
					...
					self::$wb_controls[$wid][$i]->autosize=true;
					self::$wb_controls[$wid][$i]->cursor=crHandPoint;
					self::$wb_controls[$wid][$i]->onMouseEnter=function($id){ c($id)->font->color=dsSpy::skin('AuthorColor',1); };
					self::$wb_controls[$wid][$i]->onMouseLeave=function($id){ c($id)->font->color=dsSpy::skin('AuthorColor',0); };
					...
					self::$wb_controls[$wid][$i]->cursor=crHandPoint;
					self::$wb_controls[$wid][$i]->onMouseEnter=function($id){ c($id)->font->color=dsSpy::skin('CategoryColor',1); };
					self::$wb_controls[$wid][$i]->onMouseLeave=function($id){ c($id)->font->color=dsSpy::skin('CategoryColor',0); };
					$url='http://'.self::$Host.'/'.$Text['caturi'];
					self::$wb_controls[$wid][$i]->onClick=function($id)use(&$url){ dsSpy::RunURL($url); };
					unset($url);
					// theme
					$themtxt=self::encoding_toGUI($Text['themefull']);
					self::$wb_controls[$wid][++$i]=self::ds_createLabel($parent,$x,$y+=$h,$w,$h=40,
						mb_substr($Text['themefull'],0,80).(mb_strlen($Text['themefull'])>80 ? '…' : '')
						,taCenter,fsBold,9,self::skin('ThemeColor'));
					self::$wb_controls[$wid][$i]->hint=self::encoding_toGUI($Text['themefull']);
					self::$wb_controls[$wid][$i]->wordWrap=true;
					self::$wb_controls[$wid][$i]->cursor=crHandPoint;
					self::$wb_controls[$wid][$i]->layout=tlCenter;
					self::$wb_controls[$wid][$i]->onMouseEnter=function($id){ c($id)->font->color=dsSpy::skin('ThemeColor',1); };
					self::$wb_controls[$wid][$i]->onMouseLeave=function($id){ c($id)->font->color=dsSpy::skin('ThemeColor',0); };
					...
					self::$wb_controls[$wid][$i]->cursor=crHandPoint;
					self::$wb_controls[$wid][$i]->onMouseEnter=function($id){ c($id)->font->color=dsSpy::skin('ToMessageColor',1); };
					self::$wb_controls[$wid][$i]->onMouseLeave=function($id){ c($id)->font->color=dsSpy::skin('ToMessageColor',0); };
	...
	function RunURL($url){
		if(substr($url,0,7)=='http://' || substr($url,0,8)=='https://' || substr($url,0,6)=='ftp://') shell_exec('start '.escapeshellcmd(str_replace(' ','%20',$url)));
		else return false;
		return true;
	}
	...
	function setAutoRun($ja){
		...
		$p='"'.realpath(EXE_NAME).'"';
		//pre($p);
		writeRegKey(HKEY_CURRENT_USER, 'Software\\Microsoft\\Windows\\CurrentVersion\\Run\\dsSpy', $ja ? $p : null, STRING);
		//
	}
	function ds_createTrayIconPopUpMenu(){
		...
		// разделитель
		$menu=new TMenuItem; $menu->caption=self::encoding_toGUI('-'); $popup->addItem($menu);
	...
	function CloseProgram(){
		file_put_contents(DOC_ROOT.'/files/lstpst.dat',self::SaveValue(self::$LastPost));
		file_put_contents(DOC_ROOT.'/files/wnpstns.dat',self::SaveValue(self::$WinPositions));
		file_put_contents(DOC_ROOT.'/files/vtrs.dat',self::SaveValue(self::$Avatars));
		file_put_contents(DOC_ROOT.'/files/ptns.dat',self::SaveValue(self::$Options));
	}
